...
const Funds = () => {
  ...
  const handleTransaction = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const endpoint = mode === 'DEPOSIT' ? '/portfolio/deposit' : '/portfolio/withdraw';
                   ...
                   <form onSubmit={handleTransaction} className="space-y-4">
                       <div>
                           ...
                           <input
                               ...
                               className="w-full px-3 py-2 border rounded-md bg-background text-lg font-mono"
                               value={amount}
                           ...
                           {history.map((txn) => (
                               <div key={txn.id} className="flex items-center justify-between p-4 border rounded-lg bg-card hover:bg-secondary/20 transition-colors">
                                   <div className="flex items-center gap-4">
                                       <div className={`h-10 w-10 rounded-full flex items-center justify-center ${txn.type === 'DEPOSIT' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                                           {txn.type === 'DEPOSIT' ? <ArrowDownLeft size={20} /> : <ArrowUpRight size={20} />}
                                       </div>
                                       <div>
                                           <div className="font-bold">{txn.type === 'DEPOSIT' ? 'Funds Added' : 'Funds Withdrawn'}</div>
                                           <div className="text-xs text-muted-foreground">{new Date(txn.date).toLocaleString()}</div>
                                       </div>
                                   </div>
                                   <div className={`font-mono font-bold ${txn.type === 'DEPOSIT' ? 'text-emerald-500' : 'text-red-500'}`}>
                                       {txn.type === 'DEPOSIT' ? '+' : '-'} ₹ {Number(txn.price || 0).toLocaleString() /* Assuming getTransactions returns price as total_amount logic from controller update needed? No, wait. */}
                                   </div>
                               </div>
                           ))}
                       </div>
